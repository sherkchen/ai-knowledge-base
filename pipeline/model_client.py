...
from pipeline.cost_tracker import CostTracker, cost_tracker
...
def chat(
    ...
    """Convenience function: call the default LLM and return a dict.

    Like :func:`quick_chat`, but returns a dict with ``content`` and
    ``usage`` keys so callers can access both the response text and
    token statistics.
    ...
    Returns:
        Dict with keys ``content`` (str) and ``usage`` (Usage).
    """
    messages: list[dict[str, str]] = []
    if system_prompt:
        messages.append({"role": "system", "content": system_prompt})
...
if __name__ == "__main__":
    ...
    cost_cases: list[tuple[type[OpenAICompatibleProvider], str, int, int]] = [
        ...
        (DeepSeekProvider, "deepseek-v4-pro", 2_000, 800),
